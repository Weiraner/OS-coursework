...
def verify_inodes(inode_list, filesystem):
    ...
    files = 0
    # # 在 print 之前添加调试信息
    # print(f"Verifying inodes: {inode_list}")
    # print(filesystem.diskname())
    # print(f"this is dir: {dirs}, this is files: {files}")
    for inodep in inode_list:
        inode = filesystem.read_inode(inodep)
        # print(f"inodep: {inodep},inode:{inode}")
